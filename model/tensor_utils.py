...
def get_offsets(rots, trans):
    q0 = rots[:, 0:1, :, :] # Shape: (B, 1, N, 4)
    t0 = trans[:, 0:1, :, :] # Shape: (B, 1, N, 3)
...
def get_absolute(q_rel, t_rel, ref_rigids):
    # it has been debuged that this function is exactly the inverse of get_offsets
    #  Normalize generated quaternions to prevent drift
    q_rel = q_rel / torch.linalg.norm(q_rel, dim=-1, keepdim=True)

    q0 = ref_rigids.get_rots().get_quats().unsqueeze(1) # Shape: (B, 1, N, 4)
    t0 = ref_rigids.get_trans().unsqueeze(1) # Shape: (B, 1, N, 3)
    
    # 6. PURE TENSOR COMPOSITION
    # Absolute quaternion: q_t = q0 * q_rel
    q_abs = quat_multiply(q0, q_rel)
    
    # Absolute translation: t_t = R0 * t_rel + t0
    r0_mat = quat_to_rot(q0) # Convert T=0 quat to 3x3 matrix once
    t_abs = rot_vec_mul(r0_mat, t_rel) + t0
    
    # 7. Concatenate into the final 7D trajectory
    absolute_trajectory = torch.cat([q_abs, t_abs], dim=-1)
